  ...
  return (
    <header className={`sticky top-0 ${menuOpen ? 'z-[9999]' : 'z-50'} flex items-center border-b border-gray-800 px-4 sm:px-6 md:px-10 py-4 w-full justify-between bg-gray-900/80 backdrop-blur-sm transition-transform duration-300 ${hidden ? '-translate-y-full' : 'translate-y-0'}`}>
      <div className="flex items-center gap-3">
        ...
        {pathname === '/' ? (
          <Link href="/" className="inline-flex items-center" aria-label="Upskillo home">
            <Image
              src="/logo.png"
              alt="Upskillo logo"
              width={4900}
              height={1200}
              priority
              className="h-20 md:h-14 w-[8rem] md:w-[16rem] lg:w-[12rem] object-contain"
            />
          </Link>
        ) : (
          <>
            ...
            <h2 className="text-white text-xl font-bold">Upskillo</h2>
          </>
        )}
      </div>
